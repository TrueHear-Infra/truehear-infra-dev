...
- `mgmt/local-host/`: the local-host management variant (kind-based).
  ...
  `addons/`, `infrastructure/`) with no cloud dependencies; the CAPD
  workload cluster runs 1 control plane + 3 workers.
...
- `workload/`: synced by each WORKLOAD cluster's Flux.
  - `base/`: the TrueHear application layer shared by every workload cluster
    (`truehear-platform/`: backend/vault/rabbitmq/redis Namespaces +
    ServiceAccounts; `keycloak/`: Keycloak + PostgreSQL with four
    SOPS-encrypted Secrets generated by `mise run keycloak-secrets`).
    `services/<svc>/` under each component is byte-identical to
    TrueHear/truehear-cloud-development `kubernetes/services/<svc>/`; only
    the two environment files carry `${VAR:=default}` placeholders
    (`KEYCLOAK_HOSTNAME`, `KEYCLOAK_STORAGE_CLASS`, `TRUEHEAR_ENVIRONMENT`)
    substituted from the per-cluster `cluster-vars` ConfigMap (optional).
    Base Kustomizations name a `GitRepository` source; `local-host/`
    composes `../base` and patches them to `OCIRepository`, adding
    local-path-provisioner and Podinfo. `tests/test-workload-overlays.py`
    (in `mise run validate` and CI) guards the source kinds, the defaults,
    and that every `workload/**/*.sops.yaml` is encrypted to the repo
    recipient. Workload clusters decrypt with the `sops-age` Secret that
    `mgmt/<env>/addons/flux-apps` delivers through the ClusterResourceSet
    payload `sops-age-resource-set` (created by krops-bootstrap for every
    environment).
...
- `bootstrap-rs/`: `krops-bootstrap`, the Rust CLI that ports the imperative
  ...
  The age key is required for every environment (generated with
  `mise run sops-keygen` when absent) and is also planted as the
  `sops-age-resource-set` CRS payload.
