...
The TrueHear workload layering has three levels (see
[TrueHear environments](./truehear-environments.md)):

1. `workload/base/`: environment-neutral service roots and vendored charts.
   Vendored files come from the upstream project and are never edited in
   place.
2. `workload/environments/<env>/`: the per-environment overlay, composing
   `workload/base` and carrying per-environment values, Ingress,
   HelmReleases and SOPS Secrets. Overlay Secrets are generated with
   `mise run truehear-env-secrets -- --env <env>`.
3. `workload/eu-north-1-<env>/`: the sync root of each workload cluster,
   holding only Flux Kustomizations.

To add an app to a TrueHear environment:

1. Vendor the app's neutral manifests (or chart) into `workload/base/<app>/`
   and register it in `workload/base/kustomization.yaml` when it is a
   kustomize resource.
2. Add the overlay part in `workload/environments/<env>/<app>/`: a
   `kustomization.yaml` patching the base, a HelmRelease when the app ships
   as a chart, and the SOPS Secrets generated by the mise task.
3. Add a Flux `Kustomization` row for the app to the sync root's
   `flux-ks.yaml` (name, path, `dependsOn: [truehear-platform]`, `wait`,
   `decryption.provider: sops` with `secretRef: sops-age`, health checks,
   `postBuild.substituteFrom: cluster-vars`).
4. Extend the expectations in `tests/test-workload-overlays.py`: the sync
   root's `EXPECTED_KS` name order and, when the app is its own Flux root,
   the `FLUX_ROOTS` list.
